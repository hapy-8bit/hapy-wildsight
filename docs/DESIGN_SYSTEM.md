...
- 系统安全区：冷启动时 `EntryAbility` 按当前 `colorMode` 使用与启动背景匹配的窗口、状态栏和导航栏底色，并同步浅色/深色图标；首帧之后由 `App.syncSystemBars()` 切回透明沉浸式系统栏，视觉连续且不隐藏返回手势，交互内容仍避开系统指示器。
- 图标：正式应用图标为无白边的叶形路径，已移除拍摄取景角标；使用应用纸张色作为分层背景。
- 开屏：鸿蒙系统原生启动过渡保留，普通第三方应用不尝试禁用；当前 SDK 要求 `startWindowIcon` 字段，因此继续使用 1×1 透明资源，避免系统图标与自制开屏重复闪现。应用内开屏使用正式 `foreground.png` 图标，不使用 `docs/design/icon-candidates` 候选图；外层承载面、文字和页面背景由 `AppColors` 适配浅色/深色，浅色统一为 `#F5F3EF` 系列，深色统一为 `#151A18` 系列，约 1000ms 后淡出至首页。
