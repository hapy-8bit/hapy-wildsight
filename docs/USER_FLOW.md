...
- 系统启动背景、冷启动系统栏和首屏使用当前浅色/深色主题的连续底色；深色模式首帧使用浅色系统栏图标。
- 应用内自制开屏约显示 1 秒后淡出，包含正式品牌图标“见野”和“在城市里，遇见植物”文案；图标承载面和背景随主题适配。2026-08-17 已在可用 HDC 真机分别检查浅色、深色冷启动截图和进入首页结果。
...
「设置」包含服务、存储与数据、个性化、权限与隐私、支持五组。支持组保留反馈、版本和评分，已移除“关于见野”入口；反馈问题直接进入邮件反馈。子页在设置内部以统一返回方式打开，不改变图鉴、发现、物种详情或相遇详情路由。缓存清理和本机图鉴清除分开：前者仅删除应用自有可重建缓存，后者需二次确认且不影响系统图库原图或 installationId。
