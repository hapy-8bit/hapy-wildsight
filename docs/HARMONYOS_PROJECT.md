...
- 图鉴、发现、设置三个一级页统一使用 `PageTitle`：标题与日月二级页同为 56vp 标题行、16vp 水平起点和 22fp Bold 单行文本；图鉴、发现保留标题下方的副标题，设置页不显示副标题。
- 所有二级页面统一使用 `SecondaryPageHeader`，并与日月工程的 `SettingsDetailHeader` 对齐：56vp 固定顶部栏、44vp 圆形返回按钮、23vp 左箭头、按钮与标题间距 12vp、22fp Bold 单行标题。返回按钮不增加描边或阴影，颜色继续使用见野的动态主题令牌。
- 二级标题栏与正文必须采用“固定标题栏 + 独立滚动内容区”的纵向结构，背景始终使用当前主题的不透明实色背景；不得通过滚动值控制背景、标题或返回按钮显隐。
- 设置详情、我的图鉴、物种详情、相遇详情和识别结果必须共用同一高度、返回按钮位置、标题基线和内容顶部 inset；不改变当前应用内路由和 HdsTabs。
